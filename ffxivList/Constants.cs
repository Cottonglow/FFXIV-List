using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace ffxivList
{
    public class Constants
    {
        public static string IdNotFound = "Unable to find the requested ID.";
        public static string QuestNotFound = "Unable to find the requested Quest.";
        public static string LevemeteNotFound = "Unable to find the requested Levemete.";
        public static string CraftNotFound = "Unable to find the requested Craft.";
        public static string UserNotFound = "Unable to find the requested User.";

        public static string DbUpdateError = "DbUpdateConcurrencyException occured: ";
    }
}
